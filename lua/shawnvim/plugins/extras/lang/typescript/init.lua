if shawnvim_docs then
  ...
  ---@type "vtsls" | "tsgo"
  vim.g.shawnvim_ts_lsp = "vtsls" -- currently the default

  -- To use the newer, much faster `tsgo` LSP server, either:
  -- * enable the `tsgo` extra, or
  -- * set `vim.g.shawnvim_ts_lsp = "tsgo"` in your `options.lua`
end

local extra = ShawnVim.config.register_defaults("ts_lsp", {
