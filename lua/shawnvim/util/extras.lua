...
---@class shawnvim.util.extras
local M = {}
M.buf = 0

---@type LazyExtraSource[]
M.sources = {
  { name = "ShawnVim", desc = "ShawnVim extras", module = "shawnvim.plugins.extras" },
  { name = "User", desc = "User extras", module = "plugins.extras" },
}

M.ns = vim.api.nvim_create_namespace("shawnvim.extras")
---@type string[]
M.state = nil
...
function M.wants(opts)
  ...
  if opts.root then
    ...
    return #ShawnVim.root.detectors.pattern(M.buf, opts.root) > 0
  end
  return false
end
...
function M.get()
  ...
  for _, source in ipairs(M.sources) do
    local root = ShawnVim.find_root(source.module)
    if root then
      ShawnVim.walk(root, function(path, name, type)
...
function M.get_extra(source, modname)
  ShawnVim.plugin.handle_defaults = false
  ...
  local defaults = ShawnVim.config.get_defaults()
  local def = defaults[modname]
  if def and def.enabled == false and vim.startswith(modname, "shawnvim.plugins.extras.lang.") then
    recommended = false
  end
  ...
  return {
    ...
    managed = vim.tbl_contains(ShawnVim.config.json.data.extras, modname) or not enabled,
    plugins = plugins,
    optional = optional,
  }
end
...
function X.new()
  ...
  self.float = Float.new({ title = "ShawnVim Extras" })
  ...
  for _, extra in ipairs(self.extras) do
    if extra.row == pos[1] then
      if not extra.managed then
        ShawnVim.error(
          "Not managed by ShawnExtras. Remove from your config to enable/disable here.",
          { title = "ShawnExtras" }
        )
        return
      end
      extra.enabled = not extra.enabled
      ShawnVim.config.json.data.extras = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, ShawnVim.config.json.data.extras)
      M.state = vim.tbl_filter(function(name)
        return name ~= extra.module
      end, M.state)
      if extra.enabled then
        table.insert(ShawnVim.config.json.data.extras, extra.module)
        M.state[#M.state + 1] = extra.module
      end
      table.sort(ShawnVim.config.json.data.extras)
      ShawnVim.json.save()
      ShawnVim.info(
        "`"
          .. extra.name
          ...
          .. "\nPlease restart ShawnVim to apply the changes.",
        { title = "ShawnExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("ShawnVim Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled ShawnVim extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  ...
  if not extra.managed then
    ...
    elseif vim.tbl_contains(ShawnVim.plugin.core_imports, extra.module) or origin == "default" then
      self:diagnostic({
        message = "This extra is included by default",
      })
    else
      self:diagnostic({
        message = "Not managed by ShawnExtras (config)",
  ...
  if extra.source.name ~= "ShawnVim" then
  ...
  for _, import in ipairs(extra.imports) do
    import = import:gsub("^shawnvim.plugins.extras.", "")
