...
function M.formatexpr()
  if ShawnVim.has("conform.nvim") then
...
function M.info(buf)
  ...
  ShawnVim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    ShawnVim.warn("No formatter available", { title = "ShawnVim" })
  end
end
...
function M.health()
  ...
  local has_extra = vim.tbl_contains(Config.spec.modules, "shawnvim.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    ShawnVim.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in ShawnVim.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `shawnvim.plugins.extras.lsp.none-ls` extra,",
...
function M.snacks_toggle(buf)
  return Snacks.toggle({
    ...
    get = function()
      ...
      return ShawnVim.format.enabled()
    end,
    set = function(state)
      ShawnVim.format.enable(state, buf)
    end,
  })
end
