---@class shawnvim.util.treesitter
local M = {}
...
---@return boolean ok, shawnvim.util.treesitter.Health health
function M.check()
  ...
  ---@class shawnvim.util.treesitter.Health: table<string,boolean>
  local ret = {
...
function M.build(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      ShawnVim.error(lines, { title = "ShawnVim Treesitter" })
    end
  end)
end
...
function M.ensure_treesitter_cli(cb)
  ...
  mr.refresh(function()
    ...
    if not p:is_installed() then
      ShawnVim.info("Installing `tree-sitter-cli` with `mason.nvim`...")
      p:install(
        nil,
        vim.schedule_wrap(function(success)
          if success then
            ShawnVim.info("Installed `tree-sitter-cli` with `mason.nvim`.")
            cb(true)
          else
