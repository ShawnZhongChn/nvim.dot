...
function M.setup()
  vim.schedule(function()
    if ShawnVim.config.news.shawnvim then
      if not ShawnVim.config.json.data.news["NEWS.md"] then
        M.welcome()
      end
      M.shawnvim(true)
    end
    if ShawnVim.config.news.neovim then
      M.neovim(true)
    end
  end)
end
...
function M.welcome()
  ShawnVim.info("Welcome to ShawnVim!")
end

function M.changelog()
  M.open("CHANGELOG.md", { plugin = "ShawnVim" })
end

function M.shawnvim(when_changed)
  M.open("NEWS.md", { plugin = "ShawnVim", when_changed = when_changed })
end
...
function M.open(file, opts)
  ...
  if opts.plugin then
    ...
    if not plugin then
      return ShawnVim.error("plugin not found: " .. opts.plugin)
    end
  ...
  if not file then
    return ShawnVim.error("File not found")
  end

  if opts.when_changed then
    local is_new = not ShawnVim.config.json.data.news[ref]
    local hash = M.hash(file)
    if hash == ShawnVim.config.json.data.news[ref] then
      return
    end
    ShawnVim.config.json.data.news[ref] = hash
    ShawnVim.json.save()
