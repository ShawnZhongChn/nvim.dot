---@class shawnvim.util.cmp
local M = {}

---@alias shawnvim.util.cmp.Action fun():boolean?
---@type table<string, shawnvim.util.cmp.Action>
M.actions = {
  -- Native Snippets
...
function M.expand(snippet)
  ...
  -- See: https://github.com/ShawnZhongChn/nvim.dot/issues/3199
...
function M.setup(opts)
  ...
  require("cmp.utils.snippet").parse = function(input)
    ...
    return ShawnVim.cmp.snippet_preview(input)
  end
  ...
  cmp.event:on("confirm_done", function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
      ShawnVim.cmp.auto_brackets(event.entry)
    end
  end)
  cmp.event:on("menu_opened", function(event)
    ShawnVim.cmp.add_missing_snippet_docs(event.window)
  end)
end
