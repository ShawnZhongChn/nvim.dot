...
- Python / Lua / Frontend / Rust / Markdown / Java 开发工作流优化。
...
- `lua/custom/lang/`：Python、Frontend、Rust、Markdown、Java workflow profile。
...
Java 走同一条 LSP 主路径：`filetype=java` 进入 `custom.lang.java` profile，`custom.lsp.registry` 使用 `pom.xml` / `mvnw` / `.git` 识别 Maven project root 并注册 `jdtls`，`custom.lsp.server_settings.jdtls` 集中配置 JDTLS、Maven import、Spring Boot bundles、Lombok javaagent、JDK runtime 与项目根 metadata policy。
...
- Java：`custom.lang.java` profile，JDTLS + Maven root 识别，Spring Boot tools bundle、Lombok javaagent、SDKMAN 引导的 JDK / Maven 工具链，以及 `google-java-format` 格式化策略。
...
- 主要 formatter：Lua `stylua`，Rust `rustfmt`，Java `google-java-format`，Frontend `biome` 或 `prettierd/prettier`，Markdown/YAML `prettierd/prettier`。
...
- 语言环境：Python/pip，Node.js/npm 或 yarn；Java 开发需要 JDK 21+ 与 Maven，推荐通过 `scripts/setup_java_sdkman.sh` 使用 SDKMAN 引导安装。
- Java LSP 外部工具：Mason 管理 `jdtls`、`google-java-format`、`vscode-spring-boot-tools`；项目私服依赖由用户级 `~/.m2/settings.xml` 管理，不提交到仓库。
